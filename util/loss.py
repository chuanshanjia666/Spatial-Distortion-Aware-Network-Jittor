...
Loss = box_loss (MSE on tx,ty,tw,th,the) + obj_loss (BCE).
"""
...
class SDALoss(nn.Module):
    ...
    def forward(self, predictions, targets, images):
        ...
        for s_idx, (pred, stride) in enumerate(zip(predictions, self.strides)):
            ...
            tgt = torch.zeros(B, A, F, Hs, Ws, device=device)
            obj_mask = torch.zeros(B, A, Hs, Ws, device=device)    # 1 = positive
            noobj_mask = torch.ones(B, A, Hs, Ws, device=device)
            ...
            for b in range(B):
                ...
                for box in boxes_scaled:
                    ...
                    tgt[b, best_a, 2, gy, gx] = torch.log(
                        bw / (anchor_ws[best_a] + 1e-8) + 1e-8)             # tw
                    tgt[b, best_a, 3, gy, gx] = torch.log(
